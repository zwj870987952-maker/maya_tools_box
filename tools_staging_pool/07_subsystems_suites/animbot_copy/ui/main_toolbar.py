"""
Main animBot Toolbar with:
1. Drag grip handle at the far left for moving / undocking.
2. Maya docked tabless layout (no tab headers).
3. Smooth docked edge-drag stretching & resizing (adjusts Maya splitter and toolbar height).
4. Group-Atomic Responsive Flow Layout.
5. Canonical slider mode re-ordering, 2-letter abbreviations, and 100% two-way Workspace synchronization.
"""
...
class AnimBotMainToolbar(QtWidgets.QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setObjectName("AnimBotMainToolbar")
        ...
        self.tools_map = {}       # tool_id -> QWidget
        self.groups_map = {}      # group_id -> AnimBotToolGroup
        self.ordered_groups = []  # List of groups in canonical order
        ...
        self._resize_edge = None # 'bottom', 'top', 'right', 'left', 'corner'
        self._resize_start_pos = QtCore.QPoint()
        self._resize_start_h = 44
        self._resize_start_w = 1920
        self._edge_margin = 6
        
        self._build_all_tools()
        
        # Connect workspace manager updates
        self.workspace_mgr.workspaceChanged.connect(lambda t: self.update_tool_visibility() if t == "main" else None)
        self.workspace_mgr.layoutConfigChanged.connect(lambda t: self.relayout_groups() if t == "main" else None)
    ...
    def on_slider_tool_switched(self, old_tool_id, new_tool_id):
        ...
        self.workspace_mgr.set_tool_active(old_tool_id, False, "main")
        self.workspace_mgr.set_tool_active(new_tool_id, True, "main")

    def _build_all_tools(self):
        # 0. Left Drag Grip & Logo Button Group
        grp_logo = self._create_group("grp_logo", "animBot", "#FFFFFF")
        
        self.drag_grip = AnimBotDragGrip(grp_logo)
        self.drag_grip.dragRequested.connect(self._on_grip_dragged)
        ...
        grp_nudge = self._create_group("grp_nudge", "Nudge", AnimBotColors.GREEN)
        ...
        extra_nudge = [
            ...
            ("insert_inbetween_scene", "set_inbetween_on_ones", "Insert Inbetween Scene", "bake_action"),
            ("remove_inbetween_scene", "set_inbetween_on_ones", "Remove Inbetween Scene", "bake_action"),
        ]
        ...
        grp_pose = self._create_group("grp_pose", "Default Pose", AnimBotColors.GREEN)
        pose_tools = [
        ...
        grp_bake = self._create_group("grp_bake", "Bake", AnimBotColors.GREEN)
        ...
        extra_bakes = [
            ...
            ("bake_from_last_selected", "baking_step_tangent", "Bake From Last Selected", "bake_action"),
            ("bake_infinity", "bake_infinity", "Bake Infinity Cycle", "bake_action"),
            ("share_key_times", "share_key_times", "Share Key Times", "share_keys"),
            ("share_key_times_last_selected", "share_key_times", "Share Key Times From Last Selected", "share_keys"),
            ("copy_key_times", "copy_key_times", "Copy Key Times", "share_keys"),
            ("copy_color_key_times", "copy_color_key_times", "Copy Color Key Times", "share_keys"),
            ...
            ("paste_tint_key_times", "paste_tint_key_times", "Paste Tint Key Times", "share_keys"),
        ]
        ...
        # 4. Green: Key Sliders Group (Instantiated in canonical order)
        grp_ease = self._create_group("grp_ease", "Ease", AnimBotColors.GREEN)
        ...
        grp_tween = self._create_group("grp_tween", "Tween", AnimBotColors.YELLOW)
        ...
        grp_tangents = self._create_group("grp_tangents", "Tangents", AnimBotColors.ORANGE)
        tangents_info = [
        ...
        grp_tint = self._create_group("grp_tint", "Colors", "#888888")
        ...
        grp_selection = self._create_group("grp_selection", "Purple", AnimBotColors.PURPLE)
        purple_tools = [
            ("select_sets", "select_sets", "Select Sets", "select_sets"),
            ("select_opposite", "auto_select_opposite", "Select Opposite", "select_opposite"),
            ("snapshot_opposite", "auto_select_opposite", "Snapshot Opposite Controls", "select_opposite"),
        ...
        # 9. Pink: Mirror, Align & Space Switcher Group & Sliders
        grp_mirror = self._create_group("grp_mirror", "Pink", AnimBotColors.PINK)
        pink_tools = [
            ...
            ("snapshot_mirror", "mirror_pose", "Snapshot Mirror Settings", "mirror"),
            ...
            ("copy_xform_world_space", "asset_attribute_space_switcher", "Copy Xform World Space", "space_switcher"),
            ("paste_xform_world_space", "asset_attribute_space_switcher", "Paste Xform World Space", "space_switcher"),
            ("space_switcher", "asset_attribute_space_switcher", "Selection Attribute Space Switcher", "space_switcher"),
            ("asset_space_switcher", "asset_attribute_space_switcher", "Asset Attribute Space Switcher", "space_switcher"),
        ]
        ...
        grp_temp = self._create_group("grp_temp", "Turquoise", AnimBotColors.TURQUOISE)
        turq_tools = [
            ...
            ("temp_controls_grab", "clear_temp_controls", "Temp Controls Grab Release Space", "temp_controls"),
            ("temp_controls_anim", "clear_temp_controls", "Temp Controls Anim Controls", "temp_controls"),
            ("temp_pivot", "temp_pivot", "Temp Pivot", "temp_pivot"),
            ("temp_pivot_last_object", "temp_pivot", "Temp Pivot to Last Object", "temp_pivot"),
            ("temp_pivot_centered", "temp_pivot", "Temp Pivot Centered", "temp_pivot"),
            ("temp_pivot_world", "temp_pivot", "Temp Pivot World Space", "temp_pivot"),
            ("toggle_edit_temp_pivot", "temp_pivot", "Toggle Edit Temp Pivot", "temp_pivot"),
            ("reset_temp_pivot", "temp_pivot", "Reset Temp Pivot", "temp_pivot"),
            ("global_offset", "global_offset", "Global Offset Animation", "temp_controls"),
            ("global_offset_falloff", "global_offset", "Global Offset Set Falloff", "temp_controls"),
            ("global_offset_pre_falloff", "global_offset", "Global Offset Set Pre Falloff", "temp_controls"),
            ("global_offset_post_falloff", "global_offset", "Global Offset Set Post Falloff", "temp_controls"),
        ]
        ...
        grp_bookmarks = self._create_group("grp_bookmarks", "Red", AnimBotColors.RED)
        red_tools = [
            ("motion_trail", "create_motion_trail", "Motion Trail", "motion_trail"),
            ("motion_trail_offset", "create_motion_trail", "Motion Trail Offset", "motion_trail"),
            ("motion_trail_orientation", "create_motion_trail", "Motion Trail Orientation Tracking", "motion_trail"),
            ("motion_trail_style", "create_motion_trail", "Motion Trail Style", "motion_trail"),
            ("motion_trail_frames", "create_motion_trail", "Motion Trail Frames Before After", "motion_trail"),
            ("motion_trail_space", "create_motion_trail", "Motion Trail Space", "motion_trail"),
            ...
            ("time_bookmarks_visibility", "edit_time_bookmark", "Time Bookmarks Visibility", "bookmarks"),
            ("go_to_time_bookmark", "edit_time_bookmark", "Go to Time Bookmark", "bookmarks"),
            ...
            ("edit_time_bookmark", "edit_time_bookmark", "Edit Time Bookmark", "bookmarks"),
            ("remove_time_bookmark", "edit_time_bookmark", "Remove Time Bookmark", "bookmarks"),
            ("play_keys", "bake_on_ones", "Play Keys", "bake_action"),
            ("play_color_keys", "bake_on_ones", "Play Color Keys", "bake_action"),
            ("play_on_ones", "bake_on_ones", "Play on Ones", "bake_action"),
            ("play_on_twos", "bake_on_twos", "Play on Twos", "bake_action"),
            ("play_on_threes", "bake_on_threes", "Play on Threes", "bake_action"),
            ("play_on_fours", "bake_on_fours", "Play on Fours", "bake_action"),
        ]
        ...
        grp_extra = self._create_group("grp_extra", "Extra Tools", AnimBotColors.BLUE)
        subrow_items = [
            ...
            ("cam_orient", "axis_orient_to_camera", "Axis Orient to Camera", "camera", AnimBotColors.WHITE, "white"),
            ("axis_orient_last_selected", "axis_orientation_toggle", "Axis Orient to Last Selected", "axis_toggle", AnimBotColors.BLUE, "blue"),
            ("channel_box_tools", "auto_clear_channel_box_selection", "Channel Box Extra Tools", "channel_box", AnimBotColors.BLUE, "blue"),
            ("sliders_helper", "channel_box_multi_selection_helper", "Channel Box Multi Selection Helper", "channel_box", AnimBotColors.WHITE, "white"),
            ("auto_clear_channel_box", "auto_clear_channel_box_selection", "Auto Clear Channel Box Selection", "channel_box", AnimBotColors.BLUE, "blue"),
            ("clear_channel_box", "auto_clear_channel_box_selection", "Clear Channel Box Selection", "channel_box", AnimBotColors.BLUE, "blue"),
            ("graph_editor_tools", "anim_curve_extra_tools", "Graph Editor Extra Tools", "graph_editor", AnimBotColors.BLUE, "blue"),
            ("graph_editor_toolbar_toggle", "anim_curve_extra_tools", "Graph Editor Toolbar", "graph_editor", AnimBotColors.BLUE, "blue"),
            ...
            ("anim_curve_extra_tools", "anim_curve_extra_tools", "Anim Curve Extra Tools", "graph_editor", AnimBotColors.BLUE, "blue"),
            ...
            ("cut_keys", "copy_key_times", "Cut Keys", "share_keys", AnimBotColors.GREEN, "green"),
            ("paste_keys", "paste_insert_animation", "Paste Keys", "anim_transfer", AnimBotColors.PURPLE, "purple"),
            ("paste_keys_relative", "paste_insert_animation", "Paste Keys Relative", "anim_transfer", AnimBotColors.PURPLE, "purple"),
            ("delete_keys", "auto_clear_channel_box_selection", "Delete Keys", "channel_box", AnimBotColors.BLUE, "blue"),
            ("set_smart_key", "set_inbetween_on_ones", "Set Smart Key", "bake_action", AnimBotColors.GREEN, "green"),
            ("smart_snap_keys", "set_inbetween_on_ones", "Smart Snap Keys", "bake_action", AnimBotColors.GREEN, "green"),
            ("select_objects_from_keys", "select_sets", "Select Objects From Keys", "select_sets", AnimBotColors.PURPLE, "purple"),
            ("deselect_static_objects", "select_sets", "Deselect Static Objects", "select_sets", AnimBotColors.PURPLE, "purple"),
            ("select_all_anim_curves", "anim_curve_extra_tools", "Select All Anim Curves in the Scene", "graph_editor", AnimBotColors.BLUE, "blue"),
            ("grow_keys_selection", "select_sets", "Grow Keys Selection", "select_sets", AnimBotColors.PURPLE, "purple"),
            ("shrink_keys_selection", "select_sets", "Shrink Keys Selection", "select_sets", AnimBotColors.PURPLE, "purple"),
            ("bookmark_sub", "edit_time_bookmark", "Bookmark Ribbon Submenu", "bookmarks", AnimBotColors.RED, "red"),
            ("bake_sub", "baking_step_tangent", "Baking Tools Submenu", "bake_action", AnimBotColors.RED, "red"),
            ("more_tools", "more", "More Extra Tools", "search", AnimBotColors.BLUE, "blue"),
            ("euler_e", "apply_smart_euler_filter", "Euler Filter Tool E", "euler_filter", AnimBotColors.WHITE, "white"),
            ("recovery_monitor", "anim_recovery_green", "Anim Recovery Time Machine", "recovery", AnimBotColors.WHITE, "white"),
    ...
    def _setup_logo_menu(self):
        ...
        act_pref = menu.addAction("Preferences...")
        act_pref.setIcon(AnimBotIconProvider.get_icon("preferences", "white"))
        
        act_hotkeys = menu.addAction("Set Hotkeys...")
        act_hotkeys.setIcon(AnimBotIconProvider.get_icon("hotkeys", "white"))
        
        menu.addSeparator()
        
        act_help = menu.addAction("Help & Tooltips...")
        act_tuts = menu.addAction("Interactive Tutorials")
        act_about = menu.addAction("About animBot...")
    ...
    def _on_grip_dragged(self, global_pos):
        """Handle dragging the leftmost grip handle to undock or move toolbar."""
        from .workspace_control import dock_to_location
        if self.workspace_mgr.get_location("main") != "悬浮":
            dock_to_location("悬浮")

    def set_alignment(self, alignment):
        self.workspace_mgr.set_alignment("main", alignment)

    def toggle_single_row(self):
        cur = self.workspace_mgr.get_single_row("main")
        self.workspace_mgr.set_single_row("main", not cur)

    def open_workspace_editor(self):
        """Open the visual Workspace & Tool Library Editor."""
        self.workspace_window = show_workspace_window()
        return self.workspace_window
    ...
    def update_tool_visibility(self):
        ...
        for tool_id, widget in self.tools_map.items():
            if tool_id == "logo":
                ...
                is_active = self.workspace_mgr.is_tool_active(tool_id, "main")
                widget.setVisible(is_active)
            
        # Hide/show groups based on whether any child tool is active
        for grp in self.ordered_groups:
            if grp is self.groups_map.get("grp_logo"):
                grp.setVisible(True)
                continue
            any_active = False
    ...
    def relayout_groups(self):
        """
        Group-Atomic Responsive Flow Layout Engine:
        1. Atomic grouping: An entire AnimBotToolGroup wraps together to the next line.
        2. Alignment: Left, Center, Right on each row.
        3. Single Row Mode: Arranges all in a single row and supports pan drag / mouse wheel.
        """
        margin_x = 4
        margin_y = 4
        spacing_x = 6
        ...
        is_single_row = self.workspace_mgr.get_single_row("main")
        alignment = self.workspace_mgr.get_alignment("main")
        ...
        # Multi-row wrapping mode (by entire group)
        self.pan_offset_x = 0
        lines = []
        curr_line = []
        curr_line_w = 0
    ...
    # --- Edge Resizing & Single Row Drag Panning ---
    def _detect_edge(self, pos):
        """Check if mouse position is near toolbar borders for edge stretching."""
        m = self._edge_margin
        w = self.width()
        h = self.height()
    ...
    def mousePressEvent(self, event):
        if event.button() == QtCore.Qt.LeftButton:
            ...
            if self.workspace_mgr.get_single_row("main") and self._total_content_width > self.width():
    ...
    def mouseMoveEvent(self, event):
        if self._is_edge_resizing:
            ...
            if 'right' in self._resize_edge or self._resize_edge == 'corner':
                target_w = max(200, self._resize_start_w + delta.x())
            elif 'left' in self._resize_edge:
                target_w = max(200, self._resize_start_w - delta.x())
                
            # Apply height adjustment to parent workspaceControl / Maya splitter
            p = self.parentWidget()
            while p:
                if isinstance(p, QtWidgets.QSplitter):
                    sizes = p.sizes()
                    for idx in range(p.count()):
                        child_w = p.widget(idx)
                        if child_w and (child_w.isAncestorOf(self) or child_w == self):
                            sizes[idx] = target_h
                            p.setSizes(sizes)
                            break
                    break
            ...
            # Only resize top_win if it's our own floating QDialog (NEVER resize MayaWindow)
            top_win = self.window()
            if top_win and top_win != self:
                if isinstance(top_win, QtWidgets.QDialog) and top_win.objectName() == "animBotCopyWorkspaceControl":
                    top_win.resize(target_w, target_h)
        ...
        # Update cursor for hover (Use Maya standard SplitVCursor / SplitHCursor)
        edge = self._detect_edge(event.pos())
        ...
        elif self.workspace_mgr.get_single_row("main") and self._total_content_width > self.width():
    ...
    def mouseReleaseEvent(self, event):
        if self._is_edge_resizing:
            ...
            self._is_dragging_pan = False
            self.setCursor(QtCore.Qt.OpenHandCursor if self.workspace_mgr.get_single_row("main") else QtCore.Qt.ArrowCursor)
            event.accept()
            return
        super().mouseReleaseEvent(event)

    def wheelEvent(self, event):
        if self.workspace_mgr.get_single_row("main"):
