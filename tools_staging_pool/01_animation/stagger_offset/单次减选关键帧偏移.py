...
cmds.window(window_name, title="Select Frames Tool", widthHeight=(250, 80))
cmds.columnLayout(adjustableColumn=True)

# 添加输入框
offset_value_field = cmds.floatField(value=5.0, minValue=-1000.0, maxValue=1000.0, precision=2, step=0.1)
cmds.button(label="减选物体帧", command=lambda x: deselect_and_offset_frames(cmds.floatField(offset_value_field, query=True, value=True)))
