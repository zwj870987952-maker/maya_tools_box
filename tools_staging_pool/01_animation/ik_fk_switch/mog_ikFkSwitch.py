# -*- coding: utf-8 -*-
"""
Universal IK FK Switch and Match Tool for Maya - Python 2/3 Compatible
"""
...
// www.monikagelbmann.com
	...
	b) Open Maya. In the Script Editor (Python), paste the following code:
	import pymel.core as pm
	...
	try:
		...
		pass  # Python 2 has reload as builtin
	...
	1.11 - 2025 - Python 2/3 compatibility fixes, bug fixes
...
class FkIk_UI:
	...
	def initializeTab(self, tabHeight, tabWidth):
		...
		pm.text('Knee Bend', w=frameWidth/5)
		collection2 = pm.radioCollection(win+'bendKneeAxisTfb')
		rb1 = pm.radioButton(win+'pX', label='+X', w=frameWidth/7)
		rb2 = pm.radioButton(win+'nX', label='-X', w=frameWidth/7)
		rb3 = pm.radioButton(win+'pY', label='+Y', w=frameWidth/7)
		rb4 = pm.radioButton(win+'nY', label='-Y', w=frameWidth/7)
		rb5 = pm.radioButton(win+'pZ', label='+Z', w=frameWidth/7)
		rb6 = pm.radioButton(win+'nZ', label='-Z', w=frameWidth/7)
		...
		pm.separator(h=5)
		pm.text('Release 1.10                 Monika Gelbmann                  07/2020')
		pm.separator(h=5)
...
def ikfkMatch(fkwrist, fkellbow, fkshldr, ikwrist, ikpv, switchCtrl, switchAttr, switch0isfk=1, switchAttrRange=1, rotOffset=[0,0,0], side='R', limb='arm', guessUp=1, bendKneeAxis='+X'):
	...
	if abs(pm.getAttr('%s.jointOrient%s'%(ikellbow_jnt, bendKneeAxis[1]))) < 0.1:
		...
		pm.setAttr('%s.preferredAngle%s'%(ikellbow_jnt, bendKneeAxis[1]), 12.0*multiplier)
		pm.setAttr('%s.jointOrient%s'%(ikellbow_jnt, bendKneeAxis[1]), 0.01*multiplier)
